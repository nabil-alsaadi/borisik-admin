...
> & {
  ...
  translations: {
    [languageCode: string]: ProductTranslationInput;
  };
};
...
export function getProductInputValues(
  ...
  return {
    ...
    translations: {
      ...
      [currentLanguage]: {
        name: values.translations?.[currentLanguage]?.name || values.name,
        description: values.translations?.[currentLanguage]?.description || values.description,
      },
    },
