...
function SelectCategories({
  ...
  const filteredCategories = categories.filter((cat) => cat.parent_id === null && cat.id !== initialValue?.id)
  return (
    <div>
...
export default function CreateOrUpdateCategoriesForm({
  ...
  // Watch the translated name field based on the current locale
  const translatedName = watch(`translations.${locale}.name`);

  // Use useEffect to update the slug when the translated name changes
  useEffect(() => {
    if (translatedName) {
      setSlugAutoSuggest(formatSlug(translatedName));
    }
  }, [translatedName]);
      ...
      <div className="flex flex-wrap my-5 sm:my-8">
        ...
        <Card className="w-full sm:w-8/12 md:w-2/3">
          <Input
            ...
            className="mb-5"
          />
          {console.log('errors.translations?.en?.name?.message',errors.translations?.root?.message) }
          {isSlugEditable ? (
          ...
          <div className="relative">
            ...
            <TextArea
              ...
              // {...register('details')}
              {...register(`translations.${locale}.details`)}
              variant="outline"
              className="mb-5"
            />
          </div>
